...
{
    public partial class HakoniwaPlayer : Node, IHakoObject
    {
        IHakoPdu hakoPdu;
        ...
        private async Task EventTickAsync()
        {
            ...
            if (pdu_manager == null)
            {
                GD.PushWarning("PDU Manager is null. Skipping EventTick.");
                return;
            }
...
//          Note: Rotation in Godot is in radians. 
            ...
            if (!ret)
            {
                GD.PrintErr($"FlushNamedPdu failed for {robotName} / {pdu_name}");
            }
        }
        ...
        public void EventTick()
        {
            ...
            {
                GD.PrintErr($"EventTickAsync() failed: {ex}");
            }
        }
