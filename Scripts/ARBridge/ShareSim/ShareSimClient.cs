...
{
    public partial class ShareSimClient : Node, IHakoniwaArObject
    {
        ...
        public static ShareSimClient Instance
        {
            get
            {
                if (_instance == null)
                {
                    GD.PrintErr("ShareSimClient instance is not initialized.");
                }
                return _instance;
            }
        }
        ...
        private void Awake()
        {
            ...
            {
                GD.PushWarning("Multiple ShareSimClient instances detected. Destroying duplicate.");
//                Destroy(this.gameObject);
        ...
        async void FixedUpdate()
        {
            ...
            try
            {
                ...
                foreach (var owner in owners)
                {
                    ...
                    //GD.Print($"update owner_id= {owner_id} targetOwnerId: {owner.GetTargetOwnerId()}");
                    ...
                    {
                        GD.Print($"update target owner id: new_owner_id= {owner_id} targetOwnerId: {owner.GetCurrentOwnerId()}");
                        owner.DoStop();
        ...
        public async Task<bool> RequestOwnerAsync(ShareSimObject obj, ShareObjectOwnerRequestType req_type)
        {
            ...
            if (npdu == null)
            {
                GD.Print("Can not find pdu for sharesim pdu request");
                return false;
            }
