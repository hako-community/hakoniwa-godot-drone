...
namespace hakoniwa.ar.bridge.sharesim
{
    ...
    public partial class ShareSimServer : Node, IHakoObject
    {
        IHakoPdu hakoPdu;
        ...
        private async Task DoRequestAsync(IPduManager pduManager)
        {
            ...
            if (pdu == null)
            {
                //GD.Print("Can not get pdu of req");
                return;
            }
            ...
            GD.Print("request target name: " + req.object_name);
            // find object
            ...
            {
                GD.Print($"BUSY: can not acquire owner because is target{target.GetName()} is not released. current owner is {target.GetCurrentOwnerId()}");
            }
            else
            {
                //update target owner
                ...
                target.DoStart();
                GD.Print("Updated owner: " + req.new_owner_id);
            }
            // done
        ...
        private async Task UpdateHakoTimAsync(IPduManager pduManager)
        {
            ...
            if (!ret)
            {
                GD.PrintErr($"FlushNamedPdu failed for {robotName} / {pduTime}");
            }
        ...
        private bool IsTimeout(ulong lastUpdateTime)
        {
            ...
            //GD.Print($"sim_time = {sim_time} updateTime = {lastUpdateTime}");
        ...
        public async void EventTick()
        {
            ...
            try
            {
                ...
                foreach (var owner in owners)
                {
                    ...
                    if (id == uint.MaxValue)
                    {
                        //GD.PushWarning("Why?? can not read baggage pdu...");
                    }

                    if (!owner.IsOwner(owner_id) && IsTimeout(owner.GetUpdateTime())) {
                        GD.PrintErr("ShareSim object is timeout : " + owner.GetName());
                        owner.DoStop();
            ...
            {
                GD.PrintErr($"DoRequestAsync() failed: {ex}");
            }
        }
